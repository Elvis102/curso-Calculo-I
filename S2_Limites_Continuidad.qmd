...
Al hablar del [**límite de "f" en el punto "c"**]{.alert} importa un pito si "[**c**]{.alert}" tiene imagen según "[**f**]{.alert}" o no, pues en la definición de límite de "**f**" en "**c**" solo intervienen los valores que toma "**f**" en puntos **próximos a "c"**. Así, si en todo punto "**x**" de un entorno reducido de "**c**" sucede que $f(x) = g(x)$, las funciones "**f**" y "**g**" tienen el mismo [**límite en "c"**}{.alert} ... y por eso es legítimo, [antes de calcular el **límite**]{.alert}, hacer todas las simplificaciones convenientes, incluso la supresión de factores comunes que se anulan en "**c**", siempre que estos no se anulen en las [**proximidades de "c"**]{.alert}.
...
**Simplificación y Proximidad**

En las **proximidades** del punto $x=26$, sucede que $f(x)$ coincide con la función $g(x) = 3 \cdot (x-2) + 1$. Esto es así porque, si $x$ es **próximo a 26**, se cumple que:

$\frac{3 \cdot (x-2)^{2} + {(x-2)}}{{(x-2)}} = 3 \cdot (x-2) + 1$

Se dividió el numerador ($N^{dor}$) y el denominador ($D^{dor}$) por el factor $(x-2)$, que no se anula en ningún punto de las proximidades de $x=26$.
...
::: center
::: {.bg style="background: #fdf6e3; font-size:0.78em;margin-top:-0.5em;"}
::: {.fragment}
